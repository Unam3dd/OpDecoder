...
PROJECT = libopcode
NAME = $(PROJECT).a
NAME_SHARED = $(PROJECT).so
LIBOPCODE_VERSION = 0.0.1
...
|  |__| | . |  |  | . |  _| . | . | -_|
|_____|_|___|_____|  _|___|___|___|___|
                  |_|        
...
$(LBLUE)
		Version		: $(GREEN)$(LIBOPCODE_VERSION)$(LBLUE)
...
TEST_OPCODE_OBJS = $(filter-out obj/main.o, $(OBJS))
...
$(DIR_DIST)/$(NAME): $(OBJDIR) $(OBJS)
	...
	@printf "\n[\033[0;32m\xE2\x9C\x94\033[0m] Version $(NAME)	: \033[32m$(LIBOPCODE_VERSION)\033[00m"
	@mkdir -p $(DIR_DIST)
...
$(TEST_NAME): $(INC_GTEST) $(CONTRIB_DIR) BANNER $(DIR_DIST)/$(NAME) $(OBJS) $(TEST_DIR) $(TEST_OBJS)
	@mkdir -p $(DIR_DIST)
	@$(CC) $(TEST_CFLAGS) -Itests $(TEST_OPCODE_OBJS) $(TEST_OBJS) -o $(DIR_DIST)/$(TEST_NAME)
